  ...
  return new Promise(function (resolve, reject) {
    ...
    docker.run(
      ...
      [process.stdout, process.stderr], {
        ...
        if (data.StatusCode) return reject({err: 'UserError', msg: `Evalutor failed with status ${data.StatusCode}`})
...
exports.runLinked = function (solver, evaluator) {
  return new Promise(function (resolve, reject) {
    ...
    docker.run(
      ...
      console.log(`${solver.name}: Container created`)
      ...
      return exports.run(evaluator).then(function (res) {
        ...
        // Stop and remove. Remove anyway if stop fails (probably cause already stopped)
        exports.stop(container).then(exports.remove, exports.remove).then(function () {
          reject(err)
        }).catch(reject)
      })
    })
