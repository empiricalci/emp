...
function runStandalone (experiment, logHandler) {
  var standalone = {
    Image: experiment.environment.image || experiment.environment.tag,
    WorkingDir: experiment.environment.workdir,
    HostConfig: {
  ...
  return docker.run(standalone, logHandler).then(function (res) {
    return docker.remove(res.container).then(function () {
      if (res.data.StatusCode !== 0) {
        return Promise.reject(new Error(`Container exited with code: ${res.data.StatusCode}`))
      } else {
        return res.data
      }
    })
