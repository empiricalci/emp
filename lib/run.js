...
module.exports = function (options, logger) {
  ...
  .then(function () {
    ...
    var data
    // Read from json file or directly from YML
    if (typeof experiment.dataset === 'string') data = require(path.join(options.code_path, experiment.dataset))
    if (typeof experiment.dataset === 'object') data = experiment.dataset
  ...
  .then(function (data) {
    // Attach workspace
    experiment.volumes = [`${process.env.WORKSPACES_PATH}/${experiment.id}:/workspace`]
    // Attach datasets
    if (data) {
      for (var key in data) {
        const resource = data[key]
        experiment.volumes.push(`${resource.path}:/data/${key}:ro`)
      }
    }
  })
  // Run experiment
