...
module.exports = function (options, logger) {
  ...
  .then(function () {
    ...
    experiment.directory = `${process.env.WORKSPACES_PATH}/${experiment.id}`
    mkdirp.sync(experiment.directory)
    logFile = `${experiment.directory}/experiment.log`
  ...
  .then(function (data) {
    // Attach workspace
    experiment.volumes = [`${experiment.directory}:/workspace`]
    // Attach datasets
    if (data) {
      for (var key in data) {
